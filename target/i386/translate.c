...
//    fprintf(stderr, "gen_op_mov_reg<=v memop=%u reg=%u addr=%p rax=%u rcx=%u\n", ot, reg, t0, R_EAX, R_ECX);
}
...
//    fprintf(stderr, "gen_op_mov_v<=reg memop=%u reg=%u addr=%p rax=%u rdi=%u\n", ot, reg, t0, R_EAX, R_EDI);
}
...
{
//    fprintf(stderr, "TAINT generic load idx=%d t0=%p a0=%p memindex=%u\n", idx, t0, a0, s->mem_index);
...
{
    fprintf(stderr, "FIXME: do we need to handle LODS?\n");
    gen_string_movl_A0_ESI(s);
    gen_op_ld_v(s, ot, s->T0, s->A0);
